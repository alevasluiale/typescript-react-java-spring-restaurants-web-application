  ...
  restaurants_fetching: {
    invoke: {
      ...
      onDone: {
        target: 'restaurants',
        actions: assign({
          restaurants: (context,event) => event.data.data
        })
      },
      onError: {
        target: 'home',
...
services: {
  ...
  addRestaurant:(context,event) => {
    return RestaurantService.addRestaurant((event as AddRestaurantEvent).payload.restaurant);
  },
  modifyRestaurant:(context,event) => {
    return RestaurantService.modifyRestaurant((event as ModifyRestaurantEvent).payload.restaurant);
  },
